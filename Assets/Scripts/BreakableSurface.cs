...
namespace GK {
	public class BreakableSurface : MonoBehaviour {
        ...
        // The reload function is called when the game starts and when the object is destroyed. It is used to create the mesh and collider for the object.
		public void Reload() {
			var pos = transform.position;

            // If the object does not have a mesh filter, mesh renderer, mesh collider, or rigidbody, it is added.
            if (Filter == null) Filter = gameObject.AddComponent<MeshFilter>();
            if (Renderer == null) Renderer = gameObject.AddComponent<MeshRenderer>();
            if (Collider == null) Collider = gameObject.AddComponent<MeshCollider>();
            if (Rigidbody == null) Rigidbody = gameObject.AddComponent<Rigidbody>();

            // If the object is destroyed, it is reloaded and the mesh and collider are recreated.
